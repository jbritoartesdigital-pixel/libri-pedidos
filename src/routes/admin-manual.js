...
export async function handleAdminManualApi(request, env, url) {
  ...
  if (!['full', 'reduced'].includes(body.experience)) missing.push('Experiência');
  ...
  const selection = {
    ...
    addons: {
      ...
      extraScene: optInt(body.addons?.extraScene, 0, 10) || 0,
      extraPerson: optInt(body.addons?.extraPerson, 0, 10) || 0,
    },
  };

  // Mantém a regra oficial: Confirmação Libri força Interativo.
  ...
  const valuesByColumn = {
    ...
    experience: standardQuote.experience,
